...
void ESP32MotionControl::initializeGPIO() {
    // X-axis (axis 0) - Configuration using setup section constants
    ...
    // Z-axis (axis 1) - Configuration using setup section constants
    ...
    for (int i = 0; i < 2; i++) {
        ...
        // Initialize safety limits (using setup section constants)
        float maxTravel = (i == 0) ? (float)MAX_TRAVEL_MM_X : (float)MAX_TRAVEL_MM_Z;
    ...
    Serial.printf("  X-axis: %.1f steps/mm, %.1f mm/s max, screw=%.1fmm, steps=%d\n",
                 FIXED_TO_FLOAT(axes[0].pulsesPerMM), 
                 MAX_VELOCITY_X_USER,
                 (float)SCREW_X_DU / 10000.0,
                 (int)MOTOR_STEPS_X);
    Serial.printf("  Z-axis: %.1f steps/mm, %.1f mm/s max, screw=%.1fmm, steps=%d\n",
                 FIXED_TO_FLOAT(axes[1].pulsesPerMM), 
                 MAX_VELOCITY_Z_USER,
                 (float)SCREW_Z_DU / 10000.0,
                 (int)MOTOR_STEPS_Z);
...
void ESP32MotionControl::initializePID() {
    // Initialize PID controllers for both axes using setup section constants
    for (int i = 0; i < 2; i++) {
        axes[i].pid.kP = FLOAT_TO_FIXED(PID_KP);      // Proportional gain from setup
        axes[i].pid.kI = FLOAT_TO_FIXED(PID_KI);      // Integral gain from setup
        axes[i].pid.kD = FLOAT_TO_FIXED(PID_KD);      // Derivative gain from setup
    ...
    Serial.printf("✓ PID controllers initialized: kP=%.2f, kI=%.2f, kD=%.2f\n", PID_KP, PID_KI, PID_KD);
}
